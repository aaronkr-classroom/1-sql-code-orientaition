...
SELECT 학번,
	...
	-- ROUND(SUM(기말성적)::FLOAT / COUNT(*), 2) --MYSQKL ROUND(숫자, 자릿수)
FROM 수강2
GROUP BY 학번;
...
BEGIN
	...
	OPEN ScoreListCursor; -- 커서를 열고 각 레코드(행, 튜플)를 반복
	LOOP
...
SELECT 학번, 과목번호, 평가학전, Fn_Grade(평가학전) AS 평가등급 FROM 수강2;

-- 트리거
-- 1. 새로운 테이블 생설하고 데이터 삽입
CREATE TABLE 남녀학생총수
...
CREATE OR REPLACE FUNCTION AfterInsertStudnt()
RETURNS TRIGGER AS $$
BEGIN
	IF(NEW.성별 = '남') THEN
...
EXECUTE FUNCTION AfterInsertStudnt();

-- 4. 트리거를 실행시키다 (PULL THE TRIGGER)
SELECT * FROM 남녀학생총수;
