...
2. 몽고DB의 기본 구성 요소중에서 ( collection )은 문서의 모음으로 동적 스키마를 갖는 데이블과 같다.
3. 몽고DB ( mongo shell )은 문서에 대한 질의뿐만 아니라 관리나 실행중인 인스턴스를 점검하는데 유용한 도구로써 자바스크립트 해석 기능을 가지고 있어 자바스크립트 프로그램 작성과 실행이 가능하다.
...
db.회원.countDocuments({_id: {$exists: true}});
...
문서형 (Document): JSON/BSON 문서 저장, 유연한 스키마, 예: 몽고DB

키-값형 (Key-Value): 단순 키와 값 저장, 빠른 조회, 예: Redis

열 기반형 (Column-family): 컬럼 단위 저장, 대규모 분석에 적합, 예: Cassandra

그래프형 (Graph): 노드와 관계 표현, 복잡한 연결 데이터에 적합, 예: Neo4j



**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**

몽고DB는 비관계형(NoSQL), 동적 스키마, JSON 형태 문서 저장, 수평 확장 쉬움

관계형 DB는 고정 스키마, 테이블과 관계 기반, SQL 사용, 트랜잭션과 일관성 강점

몽고DB는 유연성과 확장성에 강점이 있고, 관계형 DB는 복잡한 트랜잭션 처리와 데이터 무결성에 적합
