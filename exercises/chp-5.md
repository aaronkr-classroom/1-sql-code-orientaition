...
2. SQL은 대화식 그리고 ( 스크립트 )의 2가지 방식으로 사용할 수 있다.
...
8. ( 서브 질의 )은 질의문 안에 중첩 (nested) 되어 포함된 또 다른 SELECT 검색문을 말한다.
...
11. 중첩 질의문 중에서 내부 질의의 WHERE절 검색조건식이 외부 질의에 선언된 테이블의 일부 열을 참조하는 질의를 ( Correlated subquery )이라고 한다.
...
SELECT 학번, 이름, 나이
FROM 학생
WHERE 성별 = '남' AND 나이 = (SELECT MIN(나이) FROM 학생 WHERE 성별 = '');
...
SELECT 학번, 이름, SUM(중간성적 + 기말성적) AS 총점
FROM 학생
